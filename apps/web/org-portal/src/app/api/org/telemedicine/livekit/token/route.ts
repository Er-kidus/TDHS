...
export async function POST(request: Request) {
	try {
		...
		const upstream = await backendFetch("/telemedicine/livekit/token", {
			method: "POST",
			headers: {
				...auth,
		...
		if (payload && typeof payload === "object") {
			...
			if ("url" in asMap) {
				asMap.url = rewriteLiveKitUrlForHttpsPortal(rewriteLiveKitUrlForLan(asMap.url, request), request);
			}
			if ("serverUrl" in asMap) {
				asMap.serverUrl = rewriteLiveKitUrlForHttpsPortal(rewriteLiveKitUrlForLan(asMap.serverUrl, request), request);
			}
		}
